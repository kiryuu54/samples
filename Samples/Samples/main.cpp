...
namespace
{
	...
	float							g_PosX = 0.0f, g_PosY = 0.0f, g_PosZ = 0.0f;
...
static LRESULT CALLBACK windowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SYSKEYUP:
	{
		...
		{
			g_RotX += 1.0f;
			if (g_RotX > 360.0f) g_RotX -= 360.0f;
		}
		...
		{
			g_RotZ += 1.0f;
			if (g_RotZ > 360.0f) g_RotZ -= 360.0f;
		}
...
bool InitializeRenderResource()
{
	// 深度バッファ生成
	{
		...
		// レイアウト変更のコマンドを発行する
		{
			...
			g_VkQueue.submit(copySubmitInfo, fence);
	...
	// 頂点・インデックスバッファ生成
	{
		...
		uint32_t master[] = { 0, 1, 2, 1, 3, 2 };

		for (int i = 0; i < 6; i++)
		{
			for (int j = 0; j < 6; j++)
			{
				indexData[6 * i + j] = master[j] + 4 * i;
			}
		}
...
void DrawScene()
{
	...
	// コマンドバッファの積み込み
	{
		...
		scene.mtxProj = glm::perspectiveRH(glm::radians(60.0f), 640.0f / 480.0f, 1.0f, 100.0f);

		MeshData mesh1, mesh2;
		mesh1.mtxWorld = glm::translate(mesh1.mtxWorld, glm::vec3(g_PosX, g_PosY, g_PosZ));
		mesh1.mtxWorld = glm::rotate(mesh1.mtxWorld, glm::radians(g_RotX), glm::vec3(1.0f, 0.0f, 0.0f));
		mesh1.mtxWorld = glm::rotate(mesh1.mtxWorld, glm::radians(g_RotY), glm::vec3(0.0f, 1.0f, 0.0f));
		mesh1.mtxWorld = glm::rotate(mesh1.mtxWorld, glm::radians(g_RotZ), glm::vec3(0.0f, 0.0f, 1.0f));
		mesh2 = mesh1;
		...
		cmdBuffer.drawIndexed(36, 1, 0, 0, 1);
